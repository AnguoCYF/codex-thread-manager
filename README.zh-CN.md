...
一个纯本地、可视化、零后端依赖的 **OpenAI Codex 桌面版对话管理工具**。扫描 `~/.codex` 目录，读取 `state_5.sqlite` 中的全部对话线程，在 GUI 中列表、搜索、查看详情，并支持三档删除：**仅归档 / 数据库删除 / 完整擦除（4 处痕迹）**。提供 "Terminate Codex" 一键关闭所有 Codex 进程（删除前必须先关 Codex，否则修改会被覆盖）。
...
- 全程读写本机文件，不联网、不上传、不依赖任何第三方服务
- **跨平台**：Windows（wmic/taskkill）和 Linux（pgrep/SIGTERM）

---

## 环境要求

- Windows 10/11 或 Linux（X11）
- Python 3.8+（需包含 tkinter，官方安装包默认勾选；Debian/Ubuntu 需 `sudo apt install python3-tk`）
...
在 [Releases 页面](../../releases) 提供预编译二进制：

- `codex-thread-manager-windows.exe` — Windows 独立可执行文件，无需安装 Python
- `codex-thread-manager-linux` — Linux 独立二进制（基于 Debian 系编译）
...
| `tests/test_unit.py` | 单元测试（模块导入、函数、真实 .codex 只读校验、mock 完整删除流程） |
...
- 进程检测：Windows 用 `wmic`（约 160ms），Linux 用 `pgrep`
